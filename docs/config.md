...
| `identity.pgpPublicKeys` | list of GPG public keys | no       | Must be unique for a `repository`, only one GPG key is currently supported, standard armored string with newlines encoded as `\n` |
...
| `identity.pgpPublicKeys` | list of GPG public keys | yes      | Only one GPG key is supported.          |
