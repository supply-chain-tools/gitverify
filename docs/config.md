...
| `rules.requireSha512`          | `true`, `false` (default) | no       | Require SHA-512 in countersigned commits (via `Object-sha512: <hex>` in the mergetag, which can be created using the [pr CLI](pr.md)). SHA-512 is also required in other places like `after.sha512`, `exemptTag.hash.sha512` and SSH signatures (GPG is currently not supported). Regular tags and commits are not affected by this setting. |
