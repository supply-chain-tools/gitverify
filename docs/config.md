...
### Rules
| Config                                             | Value                      | Required | Context                                                                              | Description                                                                                                                                                                                                                                                                                                                                                                                                            |
|----------------------------------------------------|----------------------------|----------|--------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `rules`                                            | object                     | no       | -                                                                                    |                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `rules.allowSshSignatures`                         | `true` (default), `false`  | no       | -                                                                                    | `maintainers` and `contributors` are allowed to use SSH signatures                                                                                                                                                                                                                                                                                                                                                     |
| `rules.sshRequireUserPresent`                      | `true`, `false` (default)  | no       | -                                                                                    | `maintainers` and `contributors` are required to touch security key when signing. Only `sk-ssh-ed25519@openssh.com` and `sk-ecdsa-sha2-nistp256@openssh.com` is supported and will fail on other key types.                                                                                                                                                                                                            |
| `rules.sshRequireUserVerified`                     | `true`, `false` (default)  | no       | -                                                                                    | `maintainers` and `contributors` are required to use PIN with security key when signing. Only `sk-ssh-ed25519@openssh.com` and `sk-ecdsa-sha2-nistp256@openssh.com` is supported and will fail on other key types.                                                                                                                                                                                                     |
| `rules.sshKeyFormats`                              | list of string             | no       | -                                                                                    | Default/available: `sk-ssh-ed25519@openssh.com`, `ssh-ed25519`, `sk-ecdsa-sha2-nistp256@openssh.com`,`ecdsa-sha2-nistp521` , `ecdsa-sha2-nistp384`, `ecdsa-sha2-nistp256`, `ssh-rsa`                                                                                                                                                                                                                                   |
| `rules.sshHashAlgorithms`                          | list of string             | no       | -                                                                                    | `sha512` must be specified, `sha256` is optional.                                                                                                                                                                                                                                                                                                                                                                      |
...
| `rules.requireMergeCommits`                        | `true` (default), `false`  | no       | on protected branches                                                                | Require protected branches to use merge commits. Any conflicts must be resolved before merging.                                                                                                                                                                                                                                                                                                                        |
| `rules.requireTagsToBeOnProtectedBranches`         | `true` (default), `false`  | no       | annotated/lightweight tag                                                            | Require tag commits to be on a protected branch. Commits merged into a protected branch do not count, only left descendants of the branch tip. This does not apply to countersign tags.                                                                                                                                                                                                                                |
| `rules.requireCountersigning`                      | `true`, `false` (default)  | no       | on protected branches                                                                | Require protected branches to use countersinging via mergetags. The committer and tagger must be different identities. The tree of the merge commit must be the same as the tree in the tagged commit.                                                                                                                                                                                                                 |
| `rules.requireSha512`                              | `true`, `false` (default)  | no       | annotated tag, countersign commit                                                    | Require SHA-512 in countersigned commits (via `Gitverify-object-sha512: <hex>` in the mergetag, which can be created using the [pr CLI](pr.md)). SHA-512 is also required in other places like `after.sha512`, `exemptTag.sha512`. **For SSH, depending on the signature algorithm, SHA-256, SHA-384, or SHA-512 is used.** PGP is currently not supported. Regular tags and commits are not affected by this setting. |
| `rules.requireMatchedVersions`                     | `true`, `false` (default)  | no       | annotated/lightweight tag -> commit, annotated/lightweight tag -> countersign commit | Require the tag name to match `Gitverify-version` in the commit it points to. For countersign commits the version must be in the countersign tag.                                                                                                                                                                                                                                                                      |                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `rules.requireDistinctTagIdentities`               | `true`, `false` (default)  | no       | annotated tag -> commit, annotated tag -> countersign commit -> countersign tag      | The identity used to sign a tag and the commit it points to cannot be the same.                                                                                                                                                                                                                                                                                                                                        | 
| `rules.requireDistinctCountersignTagIdentities`    | `true` (default), `false`  | no       | annotated tag -> countersign commit -> countersign tag                               | The identity used to sign a countersign tag cannot be the same as the countersign commit and any tag pointing to the commit.                                                                                                                                                                                                                                                                                           |
| `rules.requireDistinctCountersignCommitIdentities` | `true` (default), `false`  | no       | annotated tag -> countersign commit -> countersign tag                               | The identity used to sign a countersign commit cannot be the same as the countersign tag and any tag pointing to the commit.                                                                                                                                                                                                                                                                                           |
...
| `exemptTag.sha1`        | git SHA-1            | yes, unless `exemptTag.sha512` is set   | Contents of `repository.exemptTags.ref`: hash of an annotated tag or a commit (for lightweight tags)                                                |
| `exemptTag.sha512`      | git SHA-512          | yes, unless `exemptTag.sha1` is set     | Contents of `repository.exemptTags.ref`: hash of an annotated tag or a commit (for lightweight tags)                                                |
